 ...
 * 
 * Modified by swordcube so you can override shit
 * AKA i got rid of inline from everything 👍
 */
class FlxTypedSpriteGroup<T:FlxSprite> extends FlxSprite {
	 ...
	 */
	public function sort(Function:Int->T->T->Int, Order:Int = FlxSort.ASCENDING):Void {
	 ...
	 * @return An iterator
	 */
	public function iterator(?filter:T->Bool):FlxTypedGroupIterator<T> {
	 ...
	 */
	public function forEach(Function:T->Void, Recurse:Bool = false):Void {
	 ...
	 * @param   Recurse    Whether or not to apply the function to members of subgroups as well.
	 */
	public function forEachAlive(Function:T->Void, Recurse:Bool = false):Void {
	 ...
	 * @param   Recurse    Whether or not to apply the function to members of subgroups as well.
	 */
	public function forEachDead(Function:T->Void, Recurse:Bool = false):Void {
	 ...
	 * @param   Recurse    Whether or not to apply the function to members of subgroups as well.
	 */
	public function forEachExists(Function:T->Void, Recurse:Bool = false):Void {
	...
	function xTransform(Sprite:FlxSprite, X:Float)
		Sprite.x += X; // addition

	function yTransform(Sprite:FlxSprite, Y:Float)
		Sprite.y += Y; // addition

	function angleTransform(Sprite:FlxSprite, Angle:Float)
		Sprite.angle += Angle; // addition

	function alphaTransform(Sprite:FlxSprite, Alpha:Float) {
	...
	function directAlphaTransform(Sprite:FlxSprite, Alpha:Float)
		Sprite.alpha = Alpha; // direct set

	function facingTransform(Sprite:FlxSprite, Facing:Int)
		Sprite.facing = Facing;

	function flipXTransform(Sprite:FlxSprite, FlipX:Bool)
		Sprite.flipX = FlipX;

	function flipYTransform(Sprite:FlxSprite, FlipY:Bool)
		Sprite.flipY = FlipY;

	function movesTransform(Sprite:FlxSprite, Moves:Bool)
		Sprite.moves = Moves;

	function pixelPerfectTransform(Sprite:FlxSprite, PixelPerfect:Bool)
		Sprite.pixelPerfectRender = PixelPerfect;

	function gColorTransform(Sprite:FlxSprite, Color:Int)
		Sprite.color = Color;

	function blendTransform(Sprite:FlxSprite, Blend:BlendMode)
		Sprite.blend = Blend;

	function immovableTransform(Sprite:FlxSprite, Immovable:Bool)
		Sprite.immovable = Immovable;

	function visibleTransform(Sprite:FlxSprite, Visible:Bool)
		Sprite.visible = Visible;

	function activeTransform(Sprite:FlxSprite, Active:Bool)
		Sprite.active = Active;

	function solidTransform(Sprite:FlxSprite, Solid:Bool)
		Sprite.solid = Solid;

	function aliveTransform(Sprite:FlxSprite, Alive:Bool)
		Sprite.alive = Alive;

	function existsTransform(Sprite:FlxSprite, Exists:Bool)
		Sprite.exists = Exists;

	function cameraTransform(Sprite:FlxSprite, Camera:FlxCamera)
		Sprite.camera = Camera;

	function camerasTransform(Sprite:FlxSprite, Cameras:Array<FlxCamera>)
		Sprite.cameras = Cameras;

	function offsetTransform(Sprite:FlxSprite, Offset:FlxPoint)
		Sprite.offset.copyFrom(Offset);

	function originTransform(Sprite:FlxSprite, Origin:FlxPoint)
		Sprite.origin.copyFrom(Origin);

	function scaleTransform(Sprite:FlxSprite, Scale:FlxPoint)
		Sprite.scale.copyFrom(Scale);

	function scrollFactorTransform(Sprite:FlxSprite, ScrollFactor:FlxPoint)
		Sprite.scrollFactor.copyFrom(ScrollFactor);
