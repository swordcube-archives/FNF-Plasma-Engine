	...
	public function changeStage(stage:String)
	{
		...
		switch (curStage)
		{
			default:
				...
				bg.loadGraphic(FunkinAssets.getImage(Paths.image('stages/stage/stageback')));
				bg.scrollFactor.set(0.9, 0.9);
				add(bg);

				stageFront = new FlxSprite(-650, 600);
				stageFront.loadGraphic(FunkinAssets.getImage(Paths.image('stages/stage/stagefront')));
				...
				stageCurtains.loadGraphic(FunkinAssets.getImage(Paths.image('stages/stage/stagecurtains')));
				...
				if (FunkinAssets.exists('assets/images/stages/$curStage/script.hx'))
				{
					trace("TRYING TO RUN SCRIPT! " + 'images/stages/$curStage/script.hx');
					script = new HScript('assets/images/stages/$curStage/script.hx');
