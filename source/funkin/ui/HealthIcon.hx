...
class HealthIcon extends FlxSprite
{
	...
	public function changeIcon(char:String = 'face')
	{
		if (this.char != char)
		{
			...
			var image = FunkinAssets.getImage(Paths.image('icons/face'));

			if (FunkinAssets.exists(Paths.image('icons/$char')))
				image = FunkinAssets.getImage(Paths.image('icons/$char'));
			...
			if (char.endsWith('-pixel'))
				...
				antialiasing = Preferences.getOption("antiAliasing");
		}
	}
}