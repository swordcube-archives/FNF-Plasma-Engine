			...
			if (AlphaCharacter.alphabet.indexOf(splitWords[loopNum].toLowerCase()) != -1 || isNumber || isSymbol)
			{
				...
				if (FlxG.random.bool(soundChance) || lastPlayed > 2)
				{
					if (playSounds)
					{
						...
						FlxG.sound.play(FunkinAssets.getSound(soundChoices[cur]));
					}
				}
				else
					lastPlayed += 1;
...
class AlphaCharacter extends FlxSprite
{
	...
	public function new(x:Float, y:Float, ?textSize:Float = 1)
	{
		super(x, y);
		this.textSize = textSize;
		var tex = FunkinAssets.getSparrow('ui/alphabet');
		frames = tex;
	...
	public function createBold(letter:String)
	{
		...
		{
			case "'":
                animation.addByPrefix(letter, "apostraphie bold0", 24);
                animation.play(letter);
                scale.set(textSize, textSize);
                updateHitbox();
				y -= 20 * textSize;
			case '-':
                animation.addByPrefix(letter, "- bold0", 24);
                animation.play(letter);
                scale.set(textSize, textSize);
                updateHitbox();
			...
			case '.':
                animation.addByPrefix(letter, "period bold0", 24);
                animation.play(letter);
                scale.set(textSize, textSize);
                updateHitbox();
				y += 45 * textSize;
