...
class Alphabet extends FlxSpriteGroup {
	...
	**/
	public function new(x:Float, y:Float, text:String = "", ?bold:Bool = false, ?textSize:Float = 1, color:FlxColor = FlxColor.BLACK, typed:Bool = false, ?typingSpeed:Float = 0.05)
	{
		super(x, y);
	...
	public static function setDialogueSound(name:String = '')
	{
		...
		soundDialog = Assets.get(SOUND, Paths.sound(name));
		if(soundDialog == null) soundDialog = Assets.get(SOUND, Paths.sound('dialogue'));
	}
...
class AlphaCharacter extends FlxSprite {
	...
	{
		super(x, y);
		var tex = Assets.get(SPARROW, Paths.image('ui/alphabet'));
		frames = tex;
